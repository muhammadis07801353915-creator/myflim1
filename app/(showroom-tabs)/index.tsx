...
import {  View, Text as RNText, SafeAreaView, FlatList, RefreshControl, ActivityIndicator, TouchableOpacity, Image, Dimensions, ScrollView, Linking  } from 'react-native';
...
export default function HomeScreen() {
  ...
  const getInterleavedData = () => {
    ...
    while (carIndex < cars.length) {
      const carGroup = cars.slice(carIndex, carIndex + (isList ? 3 : 6));
      ...
      if (!data.some(d => d.type === 'showrooms_posts') && (carIndex >= (isList ? 3 : 6) || carIndex >= cars.length)) {
        data.push({ type: 'ad', id: 'ad-main' });
        data.push({ type: 'showrooms_posts', id: 'posts-main' });
      } else if (carIndex % 12 === 0 && carIndex < cars.length) {
        ...
        >
          ...
          <View className="p-4">
            ...
            <View className="flex-row items-center mt-2">
               ...
               <Text className="text-slate-400 text-sm font-bold ml-1">{item.city || 'Iraq, Erbil, Dream City'}</Text>
            </View>
        ...
        <View className="mt-2 px-1">
          ...
          <View className="flex-row justify-between items-center mt-2">
             ...
             <View className="flex-row items-center">
                <Text className="text-slate-400 text-[10px] font-bold mr-1">{item.city || 'Erbil'}</Text>
  ...
  const renderAd = () => (
    <TouchableOpacity className="w-full h-32 bg-slate-900 rounded-[30px] my-6 overflow-hidden items-center justify-center">
       ...
       <Text className="text-white font-black text-2xl">تەبان کارس</Text>
       <Text className="text-white/70 font-bold mt-1">شوێنی متمانەی کڕین و فرۆشتن</Text>
    </TouchableOpacity>
  );
    ...
    return (
      <View className="my-6">
        <View className="flex-row justify-end items-center mb-4">
          <Text className="text-2xl font-black text-gray-900">باشترین پێشانگاکان</Text>
        </View>
        <ScrollView 
          horizontal 
          showsHorizontalScrollIndicator={false} 
            ...
            >
              ...
              <View className="p-2.5">
                ...
                <View className="flex-row justify-between items-center mt-2.5">
                  ...
                  <View className="flex-row items-center">
                    <Text className="text-slate-400 text-[9px] font-bold mr-1">{car.city || 'Erbil'}</Text>
                    <MapPin size={10} color="#94a3b8" />
                  </View>
                </View>
              </View>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListHeaderComponent={
            <>
              {renderTopSlider()}
              <View className="px-4 mt-4"><CategoryList /></View>
            </>
          }
          renderItem={({ item }) => {
