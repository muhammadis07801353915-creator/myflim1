...
export default function ShowroomChatScreen() {
    ...
    try {
      ...
      if (useSupabase && currentUserId) {
        ...
        const { data, error } = await supabase
          .from('showroom_chats')
          .select('*')
          .or(`buyer_id.eq.${currentUserId},showroom_id.eq.${currentUserId}`)
          .order('updated_at', { ascending: false });

        if (!error && data) {
          finalChats = await Promise.all(
            data.map(async (chat: any) => {
              const otherId = chat.buyer_id === currentUserId ? chat.showroom_id : chat.buyer_id;
              ...
              if (otherId) {
                const { data: profile } = await supabase
                  ...
                  .eq('id', otherId)
                  .single();
                otherName = profile?.name || profile?.phone || 'بەکارهێنەر';
              ...
              return {
                ...
                otherAvatar,
                otherId,
                myUnread: chat.buyer_id === currentUserId ? chat.buyer_unread || 0 : chat.showroom_unread || 0,
              };
            })
          );
        }
  ...
  const handleAutoStartChat = async () => {
    ...
    const showroomName = params.showroomName as string || 'بەکارهێنەر';
    const showroomAvatar = params.showroomAvatar as string || '';
    ...
    if (useSupabase && currentUserId && showroomId) {
      // === SUPABASE MODE ===
      try {
        // 1) First: try to find an existing chat for THIS EXACT car
        ...
        // Pick chat for the same car_id, or the most recent one for this showroom
        ...
        if (existing) {
          // Same car → just open, do NOT send another automated message
          ...
          if (!isSameCar) {
            // Different car from same showroom → update header and send new message
            await supabase.from('showroom_chats').update({
              car_id: carId || null,
              car_brand: carBrand,
              car_model: carModel,
              car_year: carYear,
            ...
            await supabase.from('showroom_messages').insert({
              chat_id: existing.id,
          ...
          // If same car, just open without sending a duplicate message
        } else {
        ...
        await fetchChats();
        if (chatToOpen) {
          setTimeout(() => openConversation({ ...chatToOpen, otherName: showroomName, otherAvatar: showroomAvatar }), 400);
        }
      } catch (err) {
        console.error('AutoStart Supabase error:', err);
        openLocalChat(carId, carBrand, carModel, carYear, carPrice, carImage, firstMessage, showroomName, showroomAvatar);
      }
    } else {
      // === LOCAL MODE ===
      openLocalChat(carId, carBrand, carModel, carYear, carPrice, carImage, firstMessage, showroomName, showroomAvatar);
  ...
  const openLocalChat = async (
    ...
    showroomName: string, showroomAvatar: string
  ) => {
    const saved = await AsyncStorage.getItem(getStorageKey(currentUserId));
    const currentList: any[] = saved ? JSON.parse(saved) : [];

    // Check if we already have a local chat for this seller
    const existingChat = currentList.find((c: any) => c.otherName === showroomName);
    ...
    const newChat = {
      id: `local-${Date.now()}`,
      otherName: showroomName,
      otherAvatar: showroomAvatar,
      car_id: carId,
      car_brand: carBrand,
      car_model: carModel,
      car_year: carYear,
